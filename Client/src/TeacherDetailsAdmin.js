...
class TeacherDetailsAdmin extends Component {
  constructor(props) {
    super(props);
  ...
  sendPassword = () => {
    const {username} = this.state.teacher;

    TeachersStore.newPassword(username).then((response) => {
              ...
              <Col smOffset={2} sm={10}>
                <ButtonToolbar>
                  <Button onClick={this.onUpdate} disabled={!this.state.teacher.username}>{teacherId === 'new'
                      ? 'Add'
                      : 'Update'}</Button>
                  {teacherId !== 'new' && <Button onClick={this.sendPassword} disabled={!(this.state.teacher.email && this.state.teacher.username)}>Send Password</Button>}
                </ButtonToolbar>
              </Col>
            </FormGroup>
          </Form>
...
TeacherDetailsAdmin.propTypes = {
  onChange: React.PropTypes.func,
  onSendPassword: React.PropTypes.func
};

export default withRouter(TeacherDetailsAdmin);
